...
let KEYWORDS = parseKeywords(env("KEYWORDS", ""), ["Maler", "Painter"]);
const CITY = env("CITY", "Hamburg");
const COUNTRY = env("COUNTRY", "Germany");

const TARGET_SCALE_TEXT = new RegExp(`(^|\\s)${env("TARGET_SCALE_TEXT", "500\\s*m")}(\\s|$)`, "i");
...
const PROFILE_DIR = env(
  "PROFILE_DIR",
  process.env.PPTR_PROFILE_DIR || pathSafe(path.join(__dirname, "..", "tmp", "pptr-profile"))
);
...
function saveSeenCache() {
  try {
    fs.writeFileSync(SEEN_PATH, JSON.stringify(Array.from(seenKeys), null, 2), "utf8");
  } catch {}
}
...
function metersToDegLngAtLat(m, lat) {
...
async function acceptConsentIfAny(page) {
  try {
    ...
    if (/before you continue to google/i.test(title)) {
      ...
      for (const b of btns) {
        const t = (
          await b.evaluate((el) => (el.innerText || el.getAttribute("aria-label") || "").trim())
        ).toLowerCase();
    ...
    for (const b of btns2) {
      const t = (
        await page.evaluate((el) => el.innerText || el.getAttribute("aria-label") || "", b)
      ).toLowerCase();
      if (
        /(accept all|accept|agree|alle akzeptieren|zustimmen|ich stimme zu|akzeptieren)/i.test(t)
      ) {
        await b.click().catch(() => {});
        await sleep(500);
      }
...
async function isAwSnap(page) {
  try {
    ...
    return (
      /aw,\s*snap!/i.test(txt) || /Something went wrong while displaying this webpage/i.test(txt)
    );
  } catch {
    return false;
  }
...
async function preparePageForCell(page, idx, cur) {
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      console.log(
        `[WARN] Cell ${idx + 1} browser recovery ${attempt + 1}/3: ${error?.message || error}`
      );
      try {
        page = await relaunchBrowser();
      } catch {}
...
async function recoverAwSnapAndRetry(currentPage, url, { attempts = 2 } = {}) {
  ...
  try {
    currentPage = await relaunchBrowser();
  } catch {}
  const ok = await safeGoto(currentPage, url, { retries: 1, wait: "domcontentloaded" });
...
async function ensureScale100m(page) {
  for (let tries = 0; tries < 20; tries++) {
    ...
    const zoomIn = await page.$(
      'button[aria-label*="Zoom in"], button[aria-label*="Vergrößern"], button[aria-label*="Phóng to"]'
    );
    const zoomOut = await page.$(
      'button[aria-label*="Zoom out"], button[aria-label*="Verkleinern"], button[aria-label*="Thu nhỏ"]'
    );
    if (/(\s|^)200\s*m(\s|$)|(\s|^)500\s*m(\s|$)/i.test(txt)) {
      if (zoomIn) await zoomIn.click();
    } else if (/(\s|^)50\s*m(\s|$)|(\s|^)20\s*m(\s|$)/i.test(txt)) {
      if (zoomOut) await zoomOut.click();
    } else {
      if (zoomIn) await zoomIn.click();
    }
    await sleep(500);
...
async function disableUpdateWhenMapMoves(page) {
  const found = await page.$$(
    'label[for*="EIMUcheckbox"], div[role="checkbox"], input[type="checkbox"]'
  );
  for (const el of found) {
    const text = (
      (await page.evaluate((e) => e.innerText || e.getAttribute("aria-label") || "", el)) || ""
    ).toLowerCase();
    if (
      text.includes("update results when map moves") ||
      text.includes("karte verschieben") ||
      text.includes("cập nhật kết quả khi bản đồ di chuyển")
    ) {
      const checked = await page.evaluate(
        (e) => e.getAttribute("aria-checked") === "true" || (e.tagName === "INPUT" && e.checked),
        el
      );
      if (checked) {
...
function unwrapGoogleRedirect(u) {
  try {
    ...
    if (
      /google\.(com|de|com\.vn)$/i.test(url.hostname) &&
      (url.pathname === "/url" || url.pathname.startsWith("/maps/url"))
    ) {
...
function pickBestHit(arr) {
  ...
  let candidates = arr.filter(
    (x) =>
      x && x.geojson && x.class === "boundary" && (wantedTypes.has(x.type) || x.type === "city")
  );
...
async function getCityPolygon(city) {
  ...
  const savePath =
    cacheCandidates.find((p) => p.startsWith(POLYGON_DIR + path.sep)) || cacheCandidates[0];
...
async function getDetailsFromOpenPanel(page) {
  return await page.evaluate(() => {
    ...
    out.name = (
      mainPanel?.getAttribute("aria-label") ||
    ...
    const JUNK_HOST =
      /(^|\.)(grab\.com|wa\.me|whatsapp\.com|facebook\.com|fb\.com|instagram\.com|zalo\.me|booking\.com|tripadvisor\.[a-z.]+|foody\.vn|shopee\.[a-z.]+|linktr\.ee|t\.me|tiktok\.com|youtube\.com|twitter\.com|x\.com|threads\.net|pinterest\.[a-z.]+)$/i;
    const hostOf = (u) => {
      try {
    ...
    const labelAddr = Array.from(document.querySelectorAll("[aria-label]")).find((e) =>
      /address|adresse|địa chỉ/i.test(e.getAttribute("aria-label") || "")
    );
    if (labelAddr) out.address = labelAddr.innerText || "";
    if (!out.address) {
      const blocks = Array.from(document.querySelectorAll("div,span"))
        .map((e) => e.innerText || "")
        .filter(Boolean);
      const guess = blocks.find(
        (t) => /Hamburg|Deutschland|Germany|Đức/i.test(t) && t.length < 200
      );
...
async function clearSearchBox(page) {
  const btn = await page.$(
    'button[aria-label*="Clear search"], button[aria-label*="Suche löschen"], button[aria-label*="Xóa tìm kiếm"]'
  );
  if (btn) {
...
async function searchKeywordAtCenter(page, keyword) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (again) {
        await again.click({ clickCount: 3 }).catch(() => {});
        await sleep(300);
      }
    }
  }
...
async function getSidebarHandle(page) {
  for (const s of ['div[role="feed"]', ".w6VYqd", 'div[aria-label="Results"]']) {
...
async function scrollListToEnd(page, sidebar) {
  ...
  const readVisibleLinks = async () =>
    await sidebar.evaluate((sb) =>
      Array.from(sb.querySelectorAll('a.hfpxzc, a[href*="/maps/place/"], a[href*="/place/"]'))
        .map((a) => a.href)
        .filter(Boolean)
    );
  ...
  for (let i = 0; i < SCROLL_FAST_RUNS; i++) {
    ...
    if (Date.now() - t0 > CELL_TIME_BUDGET_MS) {
  ...
  for (let i = 0; stopReason === "max_scrolls" && i < MAX_SCROLLS; i++) {
    ...
    if (stagnant >= STAGNANT_LIMIT_SLOW) {
      stopReason = "stagnant_list";
      break;
    }
    if (Date.now() - t0 > CELL_TIME_BUDGET_MS) {
      stopReason = "time_budget";
...
async function stopGracefully(signal) {
  ...
  try {
    saveCheckpoint(CURRENT_CELL_IDX, { note: `Saved on ${signal}` });
  } catch {}
  try {
    saveSeenCache();
  } catch {}
...
async function extractByClickingCards(page, sidebar) {
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    const name =
      (await card.$eval('h3,[aria-level="3"]', (el) => el.innerText).catch(() => ""))?.trim() || "";
    const placeUrl = await card
      .$eval(
        'a.hfpxzc, a[jsaction][href*="/maps/place/"], a[href*="/maps/place/"], a[href*="/place/"]',
        (a) => a.href
      )
      .catch(() => "");

    if (!placeUrl || !name || seen.has(placeUrl)) continue;
    const titleAnchor = await card.$(
      'a.hfpxzc, a[jsaction][href*="/maps/place/"], a[href*="/maps/place/"], a[href*="/place/"]'
    );
    if (titleAnchor) await titleAnchor.click().catch(() => {});
    else await card.click().catch(() => {});
    ...
    await page
      .evaluate(() => {
        (document.querySelector('[role="dialog"]') || document).scrollBy(0, 800);
      })
      .catch(() => {});
    await sleep(300);
    ...
    if (/before you continue to google/i.test(nameLower)) {
      ...
      const backBtn = await page.$(
        'button[aria-label*="Back" i], button[aria-label*="Zurück" i], button[aria-label*="Quay lại" i]'
      );
    ...
    await enqueueCsvRow({
      ...
      social: det.social || "",
      rating: det.rating || "",
      review_count: det.review_count || "",
      category: CURRENT_KEYWORD,
      city: CITY,
      google_maps_url: placeUrl,
      center_lat: CURRENT_CELL.lat,
      center_lng: CURRENT_CELL.lng,
    });
...
async function openEachPlaceAndGrab(page, links) {
  let curPage = page;
  const list =
    MAX_LINKS_PER_CELL && MAX_LINKS_PER_CELL > 0
      ? links.slice(0, MAX_LINKS_PER_CELL)
      : links.slice();
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    try {
      ...
      await curPage
        .evaluate(() => {
          (document.querySelector('[role="dialog"]') || document).scrollBy(0, 600);
        })
        .catch(() => {});
      await sleep(180);
      ...
      log.info(
        `  [COUNT] ${STT} → ${det.name} | ${det.website || "(no site)"} ${key ? "| key=" + key : ""}`
      );
...
(async () => {
  ...
  BROWSER.on("disconnected", () => {
    disconnected = true;
    console.log("[WARN] Browser disconnected");
  });
  ...
  try {
    fs.writeFileSync(POLYGON_OUT_PATH, JSON.stringify({ city: CITY, polygon }, null, 2), "utf8");
  } catch {}
  try {
    fs.writeFileSync(CENTERS_PATH, JSON.stringify(centers, null, 2), "utf8");
  } catch {}
  ...
  for (let idx = startIdx; idx < centers.length; idx++) {
    ...
    for (const kw of KEYWORDS) {
      ...
      await sleep(600);
      if (!ok) {
        console.log("  ! search box not found, skip kw");
        continue;
      }
